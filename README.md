# 📱 Welcome To Android App

This application showcases an enhanced and user-friendly Android UI layout with clean design and improved usability.Welcome To Android App ✨📱

This app demonstrates improved UI layouts (Kotlin).

## 🎨🖼️🌟 Screenshots 🌟🖼️🎨

### 🏠🔥 MAIN SCREEN 

✨ Clean Layout
✨ Smooth Navigation
!<img width="364" height="656" alt="image" src="https://github.com/user-attachments/assets/5d0d0942-8ca2-49f5-9fe2-88a548b5f8bd" />


### 🌟💫 IMPROVED UI 

💛 Better Spacing
💙 Improved Alignment
!<img width="361" height="647" alt="image" src="https://github.com/user-attachments/assets/aaec7954-b2a0-4e84-a49a-fcc30d1652bd" />

